...
- **Qwen3-TTS** — `language` takes lowercase *names* (`english`, `chinese`), not codes, or `auto`. Omitting `reference_text` transparently enables speaker-embedding-only mode.
- **dots.tts** — the runtime demands a file path for prompt audio, so the server writes a temp file; reference transcript is optional.

...
- `impl/` — seven standalone FastAPI server scripts, one per engine: `server_chatterbox.py`, `server_omnivoice.py`, `server_qwen3TTS.py`, `server_fasterQwen3TTS.py`, `server_dotsTTS.py`, `server_indexTTS.py`, `server_luxTTS.py`. Run via `python impl/server_<name>.py` or uvicorn; env config is documented in each module's docstring.
...
- Three distinct output rates: 24 kHz (Chatterbox, OmniVoice, Qwen3-TTS, faster-qwen3-tts), **48 kHz** (dots.tts, LuxTTS), 22.05 kHz (IndexTTS). dots.tts auto-selects CUDA/CPU (no device env var); its snapshot's `device` field is machine-dependent and the test deliberately compares everything *except* `device`. LuxTTS does take a `LUX_TTS_DEVICE` env var and reports the *configured* device in capabilities (pinned in `_bootstrap.py`), so its snapshot's `device` is deterministic. Don't "fix" a device mismatch in either case.
